criaCartao(
    'História',
    'Em que ano ocorreu a Proclamação da República no Brasil?',
    'Ocorreu no dia 15 de Novembro de 1889.'
)

criaCartao(
    'Química',
    'O que é uma molécula?',
    'É  um conjunto de dois ou mais átomos ligados.'
)

criaCartao(
    'Futebol',
    'O que é um penâlti?',
    'É quando um jogador é derrubado dentro da área.'
)

criaCartao(
    'Geografia',
    'Qual é o maior país em extensão territorial no mundo?',
    'O maior país é a Rússia.'
)
criaCartao(
    'Português',
    'Na frase "Os alunos chegram cedo na escola" o sujeito é?',
    'O sujeito é os alunos'
)

criaCartao(
    'Ciências',
    'Qual é a principal função dos glóbulos veremlhos no sangue humano?',
    'Transportar oxigênio dos pulmões para as células do corpo através da hemoglobina.'
)

criaCartao(
    'Física',
    'O que é a força da gravidade?',
    'É a força que atrai os corpos para o centro da Terra.'
)
criaCartao(
    'Sociologia',
    'O que é cultura na sociologia?',
    'É um conjuto de valores, crenças, hábitos, normas e conhecimentos compartilhados por um grupo social.'
)
criaCartao(
    'História',
    'Qual foi a importância da Revolução Industrial para o desenvolvimento das cidades e da sociedade no século XIX?',
    'Foi importante porque acelerou a produção, fez crescer as cidades e mudou a forma de viver e trabalhar na sociedade do século XIX.'
)